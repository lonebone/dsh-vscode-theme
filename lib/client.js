window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		(function injectCtxCss() {
			const tagId = "dsh-vscode-theme/ctx.css";
			if (typeof document !== "undefined" && document.querySelector('style[data-plugin-css="' + tagId + '"]') === null) {
				const tag = document.createElement("style");
				tag.dataset.plugin = "dsh-vscode-theme";
				tag.dataset.pluginCss = tagId;
		...
		function openEditorHeaderMenu(e, path) {
			...
			const items = [];
			if (path) {
				items.push({ label: '复制路径', run: () => { copyText(path); showToast('已复制路径: ' + path); } });
				items.push({ sep: true });
			}
			items.push({ label: '刷新文件树', run: refreshTree });
		...
		function FileNode({ name, fullPath, type, depth }) {
			...
			const onCtx = (e) => {
				...
				const items = [];
				if (isDir) {
					items.push({ label: expanded ? '折叠' : '展开', run: () => setExpanded(!expanded) });
					items.push({ label: '在资源管理器中定位', run: () => revealPath(fullPath) });
					items.push({ sep: true });
				} else {
					items.push({ label: '打开', run: () => { onClick({ ctrlKey: false, metaKey: false, shiftKey: false }); } });
					items.push({ label: '复制文件名', run: () => { copyText(name); showToast('已复制文件名: ' + name); } });
					items.push({ label: '在资源管理器中定位', run: () => revealPath(fullPath) });
					items.push({ sep: true });
				}
				const n = (vscodeState.selectedPaths || []).length;
				items.push({ label: n > 1 ? ('复制相对路径 (' + n + ')') : '复制相对路径', run: () => copySelected(false) });
				items.push({ label: n > 1 ? ('复制绝对路径 (' + n + ')') : '复制绝对路径', run: () => copySelected(true) });
				items.push({ sep: true });
				items.push({ label: '刷新文件树', run: refreshTree });
				openCtxMenu(e.clientX, e.clientY, items);
			};
		...
		function ScmFileRow({ f, vscode, onOpen, onOp }) {
			...
			return React.createElement("div", {
				...
				onContextMenu: (e) => {
					...
					e.stopPropagation();
					const items = [];
					items.push({ label: '打开', run: () => onOpen(f) });
					items.push({ label: '在资源管理器中定位', run: () => revealPath(f.path.startsWith('/') || /^[a-zA-Z]:/.test(f.path) ? f.path : joinPath(vscode.rootPath || vscode.repoPath, f.path)) });
					items.push({ sep: true });
					const untracked = f.code[0] === '?';
					if (f.staged) {
						items.push({ label: '取消暂存', run: () => onOp('unstage', f) });
					} else {
						items.push({ label: '暂存更改', run: () => onOp('stage', f) });
					}
					items.push({ label: untracked ? '放弃更改（删除文件）' : '放弃更改', danger: true, run: () => onOp('discard', f) });
					if (untracked) items.push({ label: '添加到 .gitignore', run: () => onOp('ignore', f) });
					openCtxMenu(e.clientX, e.clientY, items);
				},
				title: f.path
			},
		...
		function EditorPane() {
			...
			if (vscode.commitDiff) {
				...
				return React.createElement("div", { className: "vsc-editor" },
					React.createElement("div", { className: "vsc-editor__header", onContextMenu: (e) => openEditorHeaderMenu(e, cd.file) }, "commit " + cd.hash + " \u2014 " + cd.file),
					cd.text === null
				...
				tabs.map((id) => {
					...
					return React.createElement("div", {
						...
						onContextMenu: (e) => {
							...
							e.stopPropagation();
							const items = [];
							if (id !== current) items.push({ label: '切换到该会话', run: () => openSession(id) });
							items.push({ label: '关闭标签', run: () => setVscode({ openTabs: vscodeState.openTabs.filter((x) => x !== id) }) });
							items.push({ label: '复制标题', run: () => { copyText(title); showToast('已复制: ' + title); } });
							openCtxMenu(e.clientX, e.clientY, items);
						},
						title: title
					},
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				const onCtx = (e) => {
					...
					if (t.closest('.vsc-treenode, .vsc-scm__file, .vsc-tab, .vsc-editor__header')) return;
					const frame = t.closest('.vsc-frame');
					if (!frame) return;
					if (!t.closest('.vsc-activitybar, .vsc-sidebar, .vsc-tabs')) return;
					e.preventDefault();
					e.stopPropagation();
					const items = [];
					items.push({ label: '刷新文件树', run: refreshTree });
					if (vscodeState.selectedPath) {
						items.push({ label: '复制当前文件路径', run: () => copySelected(false) });
					}
					items.push({ sep: true });
					items.push({ label: '切换 Enter 模式: ' + (vscodeState.enterMode === 'newline' ? '回车换行' : '回车发送'), run: () => setEnterMode(vscodeState.enterMode === 'newline' ? 'send' : 'newline') });
					items.push({ label: '打开 DSH 设置', run: openSettingsDialog });
					openCtxMenu(e.clientX, e.clientY, items);
				};
